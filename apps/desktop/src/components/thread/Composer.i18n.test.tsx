...
describe("LiveSessionPage strings (i18n)", () => {
  it("renders the disconnected-runtime card in English (no Tauri sidecar in tests)", async () => {
    ...
    renderAt("/live");
    expect(await screen.findByText("Qoder runtime")).toBeInTheDocument();
    expect(
      screen.getByText((_, node) =>
        node?.textContent === "The desktop app runs a bundled Qoder automatically. In the browser, start one with opencode serve and connect.",
      ),
    ).toBeInTheDocument();
  });
});
