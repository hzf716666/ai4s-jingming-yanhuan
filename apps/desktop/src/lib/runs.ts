...
// records in `.jingming-yanhuan/runs.jsonl`. Unlike file provenance (which records
...
export function runInputFromEvent(event: ToolUpdatedEvent): RunInput | null {
  ...
  // record them instead (into .jingming-yanhuan/remote-runs.jsonl) with real remote
...
export function reproduceRunPrompt(r: RunRecord): string {
  ...
  if (env) {
    ...
    if (env.packages)
      parts.push(
        `The environment had ${env.packages.count} installed Python packages, pinned in \`.jingming-yanhuan/env/${env.packages.hash}.txt\` — install matching versions from that lockfile if the result differs.`,
      );
  }
