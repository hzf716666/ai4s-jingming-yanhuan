...
// <workspace>/.jingming-yanhuan/compute.json (see compute_machines below).
...
fn compute_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    // agent reaches it from its session folder via `../.jingming-yanhuan/compute.json`
    // (the seeded harness points at it), so no per-session copies are kept.
    Ok(base_workspace_dir(app)?.join(".jingming-yanhuan").join(COMPUTE_FILE))
}
...
fn load_machines(app: &AppHandle) -> Result<Vec<Machine>, String> {
    ...
    let legacy = base_workspace_dir(app)?.join(".jingming-yanhuan").join("hpc.json");
...
/// workspace's `.jingming-yanhuan/compute.json` so the agent reads it IN-workspace —
...
pub(crate) fn materialize_active(app: &AppHandle) {
    ...
    write_materialized(
        &base.join(".jingming-yanhuan").join(COMPUTE_FILE),
        &active.join(".jingming-yanhuan"),
    );
}
...
";
    ...
    fn materialize_copies_then_clears() {
        ...
        let base_os = root.join("base-jingming-yanhuan");
        let sess_os = root.join("session-jingming-yanhuan");
