---
name: 可追溯性审计
description: 审查报告/论文/分析的可追溯性：每个结论都能回溯到数据、代码与来源
---
...
```bash
python "$XDG_CONFIG_HOME/opencode/skills/可追溯性审计/pdf_extract.py" MANUSCRIPT.pdf
```
...
1. Read `.jingming-yanhuan/provenance.jsonl` in the workspace — one JSON record per
   ...
   the file directly (`cat .jingming-yanhuan/provenance.jsonl`) instead of relying
