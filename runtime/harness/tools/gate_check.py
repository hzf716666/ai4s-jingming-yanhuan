...
def check_p2_5(proj: Path) -> list[str]:
    ...
    if not gaps:
        return []
    fails = []
    report = proj / "data_gap_report.md"
    supp = read_json(proj / "data_supplement.json") or {}
    has_supplements = bool(supp.get("supplements"))
    if not report.is_file() and not has_supplements:
        fails.append(f"{len(gaps)} 个 A 档子问题标了 data_gap 但无 data_gap_report.md/补充数据 —— 未执行 P2.5 数据缺口寻找")
    return fails
...
ORDER = ["p1", "p1_5", "p2", "p2_5", "p3", "p4", "p5", "p6", "p7"]
STAGE_LABEL = {
    "p1": "P1 拆解", "p1_5": "P1.5 文献检索", "p2": "P2 过滤",
    "p2_5": "P2.5 数据缺口寻找", "p3": "P3 数据盘点", "p4": "P4 实验执行",
    "p5": "P5 结果整合", "p6": "P6 论文写作", "p7": "P7 自检评审",
}
