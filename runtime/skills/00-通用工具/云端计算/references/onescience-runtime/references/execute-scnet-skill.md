...
进入本执行分支前必须已有当前轮次 preflight 证据：`preflight_passed=true`、`execution_readiness=ready`、`evidence.preflight.status=passed`、`evidence.preflight.conda_checked=true`、`evidence.preflight.environment_checked=true`、`evidence.preflight.channel_checked=true`、`evidence.preflight.entrypoint_checked=true`。任一缺失或失败时，不得委托 `超算平台助手` 提交新任务，必须回到 `preflight`；若缺少 `runtime.conda` 或目标环境未 ready，立即委托 `onescience-installer`。

## Routing Boundary

`scnet_skill` 表示 `run_site=remote + access_mode=scnet` 的 SCnet 平台执行上下文；`execution_mode` 可以为空、`none` 或 `slurm`。当 `execution_mode=slurm` 时，runtime2 仍然委托 `超算平台助手` 提交任务，只把队列和资源字段整理为 超算平台助手 参数，不在本地或远端直接执行 `sbatch`。

命中以下平台动作时，继续直接委托 `超算平台助手`：
...
- 整理交接输入给 `超算平台助手`
- 消费 `超算平台助手` 返回的状态、日志、阻断与平台错误
- 把结果归一化为 runtime 输出

## SCnet Config Handoff

凡是通过 `超算平台助手` 提交新任务，必须先读取 `onescience.json.runtime.scnet` 并构造结构化交接。不要让 `超算平台助手` 根据缓存默认区域、默认队列或自然语言猜测提交目标。
...
- `partition` 归一为 超算平台助手 的 `queue` / `--queue`。
- `remote_work_dir` 归一为 超算平台助手 的 `work_dir` / `--work-dir`。
- `time_limit` 归一为 `wall_time` / `--wall-time`。
- `memory` 归一为 `job_mem` / `--job-mem`。
- 若 `scnet.region` 存在，交接给 超算平台助手 时必须显式包含区域；可以先请求 `超算平台助手` 切换到该区域，或使用支持区域前缀的命令格式。
- 若 `runtime.scnet` 中缺少提交必需字段，立即转交 `onescience-runsite` 补齐配置，不能退回到 `runtime.cluster.*` 或 超算平台助手 默认值。
...
6. 先优先询问 `超算平台助手` 是否可自动生成 `run_job.slurm`；如果可以，直接让 `超算平台助手` 在远端生成并提交。
7. 如果 `超算平台助手` 未自动生成 `run_job.slurm`，runtime2 必须参考 `./assets/templates/slurm_cpu.sh`、`./assets/templates/slurm_dcu.sh`、`./assets/templates/slurm_gpu.sh`、`./assets/templates/slurm_gpu_multinode_torchrun.sh` 或 `./assets/tpl.slurm`，再根据 `runtime.cluster.*`、`runtime.target.*`、`runtime.modules`、`runtime.script.*`、`runtime.env_vars.*` 和 `runtime.conda.enabled` 在本地生成同名脚本，并上传到同一远端目录。
8. 如果 `runtime.conda.enabled=true`，远端脚本中保留 `runtime.conda.activate_script`；如果 `runtime.conda.enabled=false`，远端脚本中跳过激活步骤。
9. 通过 `超算平台助手` 触发远端提交、状态查询和日志下载；提交命令必须包含来自 `onescience.json.runtime.scnet` 的区域、队列和工作目录，例如 `在{region}提交作业 bash run_job.slurm --queue {partition} --work-dir {scnet.remote_work_dir} --ppn {ppn} --wall-time {wall_time} --job-name {job_name}`。
