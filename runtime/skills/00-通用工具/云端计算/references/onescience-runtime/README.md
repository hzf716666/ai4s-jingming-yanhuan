...
- `runtime.scnet.*`（当 `execution_channel=scnet_mcp` 或后续需要委托 `超算平台助手` 提交任务时，这是 SCnet region/partition/work_dir/资源参数的唯一主来源）
- `runtime.modules.*`
...
- `scnet_mcp`：交接给 `超算平台助手` 时显式要求平台日志下载到当前测试目录的 `logs/`，并在 runtime 输出中记录 `local_log_dir=<work_dir>/logs/`。
...
- 命中 SCnet 作业、文件、账户、区域、队列、集群、日志下载等平台动作时，继续委托 `超算平台助手` 技能执行；runtime 只负责交接输入、消费结果与基础诊断。
- 通过 `超算平台助手` 提交任务前，必须先读取 `onescience.json.runtime.scnet` 中的 `region`、`partition`/`queue`、`remote_work_dir`/`work_dir`、资源参数和作业名等信息；`partition` 归一为 超算平台助手 的 `--queue` 参数。不要依赖 超算平台助手 的缓存默认区域或默认队列，也不要用用户自然语言里的 region/partition 直接覆盖该配置。
