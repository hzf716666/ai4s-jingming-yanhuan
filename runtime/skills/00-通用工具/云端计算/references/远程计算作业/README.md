---
name: 远程计算作业
description: 通过 SSH 在远程 GPU/计算集群上提交、监控、取消作业，管理远程环境
---
...
1. `cat .jingming-yanhuan/compute.json` in the workspace (the app keeps this file in
...
1. Pick a job name and build the remote dir path (remember the literal string —
   ...
   `REMOTE=jingming-yanhuan/jobs/<name>-<YYYYmmdd-HHMMSS>`
...
2. Submit:
   ```bash
   REMOTE=jingming-yanhuan/jobs/<job-name>-$(date +%Y%m%d-%H%M%S)
...
```bash
python "$XDG_CONFIG_HOME/opencode/skills/远程计算作业/record_run.py" \
  ...
  --session-id "$(cat .jingming-yanhuan/session.txt 2>/dev/null)"
```
