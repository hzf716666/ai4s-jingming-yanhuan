...
本地通道只能使用本地 shell 证据；SSH/远端 SLURM 通道必须通过 SSH 在远端执行探测；SCnet 通道必须通过 `runtime.scnet` 和 `超算平台助手` 可消费的显式交接信息确认。不得用本地 import、历史日志或自然语言线索替代目标环境发现结果。
...
当 `execution_channel=scnet_mcp` 且用户意图包含”提交新任务”时，runtime2 必须读取 `onescience.json.runtime.scnet`，并把它作为交给 `超算平台助手` 的提交参数来源。
...
- `region` 必须来自 `runtime.scnet.region`，用于 超算平台助手 的区域选择或区域化提交。
- `partition` 是 OneScience 侧规范字段；提交给 超算平台助手 时归一为 `queue` / `--queue`。若只存在 `scnet.queue`，可作为兼容别名读取，但输出仍记录 `partition`。
- `remote_work_dir` 与 `work_dir` 二选一；提交给 超算平台助手 时归一为 `--work-dir`。
- `nodes`/`nnode`、`ppn`/`cpus_per_task`、`gpus_per_node`、`dcus_per_node`、`memory`/`job_mem`、`time_limit`/`wall_time`、`job_name` 按 超算平台助手 支持的显式参数传递。
...
- 若用户意图是提交新任务，确认 `scnet.region`、`scnet.partition`/`scnet.queue`、`scnet.remote_work_dir`/`scnet.work_dir` 可用，并准备交给 `超算平台助手` 的显式参数，不允许让 超算平台助手 自动选择默认区域或默认队列。
- 若用户意图是观察已有任务，确认 `task_id` 或其等价标识可用。
- 平台侧真实动作继续交给 `超算平台助手`，preflight 只判断交接是否完整。
