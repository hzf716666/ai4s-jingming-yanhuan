...
name: 超算平台助手
type: executor
description: SCNet 超算平台管理：查询/提交作业、文件管理、账户与区域切换
version: "1.0.0"
source: oneskills
---
...
2. **检查配置**：确保 `~/.超算平台助手.env` 文件存在且包含必要的环境变量
...
- 启动命令只能出现在 `提交作业` 后、第一组显式参数前，例如 `bash run.sh`；`--queue`、`--work-dir`、`--ppn`、`--wall-time`、`--job-name` 等必须作为 超算平台助手 参数单独传递，不能拼进启动命令。
...
超算平台助手 依赖远程 API，网络不稳定时可能出现超时或连接失败。执行命令时必须遵守以下规则：
...
2. **用户目录 `~/.超算平台助手.env`**（兼容方式）
   - 手动配置文件
   - 用于独立使用 超算平台助手 或临时覆盖配置
...
#### 方式 2：手动配置 ~/.超算平台助手.env

首次使用可手动在 `~/.超算平台助手.env` 配置：
