...
name: 自主科研流水线
type: knowledge
description: 从主题、配置与输出目录运行自主科研流水线
version: "1.0"
...
1. Verify config file exists:
   ```bash
   ls config.yaml || ls config.自主科研流水线.example.yaml
   ```
2. If no `config.yaml`, create one from the example:
   ```bash
   cp config.自主科研流水线.example.yaml config.yaml
   ```
...
```bash
自主科研流水线 run --topic "Your research topic here" --auto-approve
```
...
```python
from 自主科研流水线.pipeline.runner import execute_pipeline
from 自主科研流水线.config import RCConfig
from 自主科研流水线.adapters import AdapterBundle
...
```python
from 自主科研流水线.pipeline.runner import execute_iterative_pipeline
...
- **Config validation error**: Run `自主科研流水线 validate --config config.yaml`
