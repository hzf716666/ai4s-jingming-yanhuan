...
name: 论文结构规划
type: expert
description: 论文结构规划：规划 IMRAD 结构、图表位置与论证逻辑线
version: "1.0.0"
...
- ✅ 设计论证逻辑线
- ✅ 选择目标会议/期刊模板
- ❌ 不写论文正文（由 论文初稿 executor 执行）
- ❌ 不审稿（由 同行评审 executor 执行）

## 输入
orchestrator 传入: task_state, analysis_report, experiment_results, figures, hypotheses

## 输出: planner_proposal
{ paper_outline: { sections: [{title, key_message, figures, tables, word_budget}], narrative_flow: "问题→现有方法局限→我们的方法→实验验证→结论" }, target_venue: "NeurIPS | ICML | ICLR | Nature", format: "LaTeX", suggested_executor: "论文初稿" }
