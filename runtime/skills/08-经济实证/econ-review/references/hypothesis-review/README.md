...
- 观测数据场景: 禁止因果断言
- 不编造文献; 检索不到就写"文献检索受限"
- 报告落盘 `review_report.md`(与 econ-stat-review 产物同目录, 但本报告是**开工前想法评审**)
- **数据缺口必补(新增)**: 评审若发现【本地数据不可核验 X 指标】或【某证据依赖外部数据】, 必须在"强化建议"第 1 条写"补采外部数据", **并立即调用技能 econ-data-fill** 检索补充(统计局/OECD/清科/数据库), 产出 data_gap_report.md + data_supplement.json; 找不到才允许在"限制"里说明。评审后接正式研究时, 把缺口处理清单传给 P1(标"外部补充已做/未做")。
