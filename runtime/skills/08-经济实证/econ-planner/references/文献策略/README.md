...
name: 文献策略
type: expert
description: 文献搜索策略：关键词扩展、数据库选择、布尔查询构建
version: "1.0.0"
...
- ✅ 预估搜索覆盖率和返回量
- ✅ 建议筛选标准和去重策略
- ❌ 不实际检索文献（由 文献收集 executor 执行）
- ❌ 不筛选文献（由 文献筛选 executor 执行）

## 输入
orchestrator 传入: task_state, available_resource_summaries, research_questions

## 输出: planner_proposal
{ search_plan: { databases, queries, keywords: {expanded, synonyms}, filters: {year, language, type} }, expected_coverage, dedup_strategy, suggested_first_executor: "文献收集" }
