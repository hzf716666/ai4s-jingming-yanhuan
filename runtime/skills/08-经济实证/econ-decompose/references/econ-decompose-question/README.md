...
5. **标 gap 即补(新增)**: 任何子问题标了 `data_gap: true`, 产出 sub_problems.json 后**立即调用技能 econ-data-fill** 去外部源找该指标(不等 P2/P2.5), 找到的数据追加进 data/records.json; 找不到在 data_gap_report.md 记录尝试过的源, 并告知用户"该子问题待外部数据"。
6. 产出 `sub_problems.json`,写回项目根,在其后简短汇报(子问题数/A/B/C 预判分布 + 缺口处理结果)。
