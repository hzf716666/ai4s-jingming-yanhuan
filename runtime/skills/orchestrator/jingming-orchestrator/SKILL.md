---
name: jingming-orchestrator
type: orchestrator
description: 景明研环CIENCE 统一编排主控。五阶段工作流：INTAKE→RESOURCE→EXPERT→PLAN→EXECUTE。融合 OneSkills 契约系统 + ARC 方法论 + 动态路由中断处理。
version: "1.0.0"
...
{
  "event": "progress",
  "current_skill": "literature-collect",
  ...
  "phase_skills": {
    ...
    "current": "literature-collect",
    "pending": ["literature-screen", "knowledge-extract"]
  }
}
```
...
Phase 3 "实验"     ← design + execution + analysis
  experiment-design → code-generation → resource-planning →
  experiment-run → iterative-refine → result-analysis → research-decision
...
│                                                             │
│ 当前可用的 expert:                                           │
│   来自 ARC: research-scoper, literature-strategist,          │
│             knowledge-synthesizer, hypothesis-generator,     │
│             experiment-planner, research-decider,            │
│             paper-structural-planner                         │
