#!/usr/bin/env python3
"""Record a remote (SSH/HPC/Modal) experiment run into the 景明研环 provenance.
...
record to <workspace>/.jingming-yanhuan/remote-runs.jsonl, which the app merges into
...
STORE = os.path.join(".jingming-yanhuan", "remote-runs.jsonl")
ENV_DIR = os.path.join(".jingming-yanhuan", "env")  # package lockfiles, content-addressed
...
def read_env(path):
    ...
    .jingming-yanhuan/env/<hash>.txt (mirroring how the app records local runs), and
    ...
    env: dict[str, object] = {
        ...
        "app": os.environ.get("JINGMING_APP_VERSION", "unknown"),
    }
    if python:
        env["python"] = python
    if freeze:
...
def main():
    p = argparse.ArgumentParser(description="Record a remote run into 景明研环 provenance.")
    ...
    # The skill passes `--session-id "$(cat .jingming-yanhuan/session.txt)"`, which is
